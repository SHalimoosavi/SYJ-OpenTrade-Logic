...
[![CI](https://github.com/SHalimoosavi/SYJ-OpenTrade-Logic/actions/workflows/ci.yml/badge.svg)](https://github.com/SHalimoosavi/SYJ-OpenTrade-Logic/actions/workflows/ci.yml)
...
```
syj-opentrade-logic/
├── core/                       # Zero-dependency logic modules
│   ├── models.py               #   HTS tree + result dataclasses
│   ├── gri_engine.py           #   GRI 1 / 3(a) / 6 DAG traversal + stemming
│   ├── rulings_search.py       #   BM25 lexical search over CROSS rulings
│   └── duty_calculator.py      #   Section 301/232 + AD/CVD estimation
├── cli/
...
│   ├── main.py                  #   app entrypoint, classify endpoint
│   ├── database.py               #   SQLAlchemy models (Org/User/Product/AuditLog/Webhook)
│   ├── security.py, dependencies.py  #   JWT auth, RBAC
│   ├── routes_*.py               #   auth, catalog, org, rulings, duty, webhooks, audit, reports
│   ├── webhooks.py, webhook_triggers.py  #   HMAC-signed delivery
│   ├── reports.py                #   CSV/Excel/PDF generation
│   ├── requirements.txt          #   production dependencies
│   ├── requirements-dev.txt      #   test-only dependencies (pypdf, httpx)
│   └── test_*.py                 #   full integration test suite
├── frontend/                   # v0.5.0 — Vite + React + TypeScript dashboard
│   └── src/{pages,components,lib,types}/
├── scripts/
│   └── import_hts_data.py      # Pulls full 99-chapter HTS from the live USITC API
├── data/
│   ├── hts_sample.json          # Small demo dataset
│   ├── hts_full.json            # Generated by the importer — not committed
│   ├── cross_rulings_sample.json
│   ├── trade_programs_sample.json
│   └── adcvd_sample.json
├── tests/                       # Core engine test suites
├── .github/workflows/ci.yml     # v0.9.0 — runs the full suite + real Docker builds
...
| **v0.7.0** | Duty calculator (Section 301/232 + AD/CVD scope flagging) | ✅ Done — 19 new tests passing here; real, dated, sourced sample data with strong disclaimers |
| **v0.8.0** | Reports (PDF/CSV/Excel), audit trails, webhooks | ✅ Done — 103 tests passing here, including real HTTP webhook delivery to a local server and round-tripped PDF/Excel files |
| **v0.9.0** | Docker/Compose, GitHub Actions CI/CD, deployment docs | ✅ Done — see [`DEPLOYMENT.md`](./DEPLOYMENT.md); CI runs for real on GitHub's servers on every push |
...
```bash
# Install both production and test-only dependencies
pip install -r server_fastapi/requirements.txt --break-system-packages
pip install -r server_fastapi/requirements-dev.txt --break-system-packages

# Core engine, importer, rulings search, duty calculator (no FastAPI app instantiated)
python3 -m unittest tests.test_gri_engine tests.test_api tests.test_importer_and_terse_headings tests.test_rulings_search tests.test_duty_calculator -v

# Standalone server modules (security, CSV/Excel import, webhooks, reports)
python3 -m unittest server_fastapi.test_security server_fastapi.test_catalog_import server_fastapi.test_webhooks server_fastapi.test_reports -v

# Each of these instantiates a real FastAPI app + SQLAlchemy engine and
# MUST run as a separate command -- combining them in one invocation
# silently breaks database isolation between them (a real bug found
# during v0.4.0 development; see .github/workflows/ci.yml's comments)
python3 -m unittest server_fastapi.test_main -v
...
**103 tests, all passing**, including: correct GRI-based classification, refusal to guess on unmatched products, decision-path integrity, live HTTP integration against a real running server, direct SQLite file verification, real-data regressions found while importing the live USITC feed (chapter detection, terse headings, a stemmer bug affecting silent-e plurals), BM25 rulings search ranking, duty calculation across Section 301/232 and AD/CVD scenarios, real HMAC-signed webhook delivery to an actual local HTTP server with independent signature verification, and real generated PDF/Excel files round-tripped and verified.

CI runs this full suite automatically on every push (see the badge above and [`.github/workflows/ci.yml`](./.github/workflows/ci.yml)), plus a real `npm run build` of the dashboard and real `docker build` of both production images on GitHub's Linux servers.

---

## 🤝 Contributing

This project is built in the open, in public, with every bug and fix documented rather than hidden. CI runs automatically on every push and pull request (see [`DEPLOYMENT.md`](./DEPLOYMENT.md)). Issues and PRs welcome.
