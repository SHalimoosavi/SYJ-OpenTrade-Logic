...
**Baseline: v0.9.1 + v0.9.2 dashboard polish**
**Status: Dashboard polish build verified**
**Date: 2026-08-09**
...
- 🟢 Recent polish pass (code-splitting, dark/light mode, toast
  ...
  Product Catalog pagination) is complete and the final `npm run build`
  has been successfully verified after the Command Palette + Breadcrumbs
  integration.
...
- 🟢 Bundle size was 887kB as a single chunk before code-splitting was added;
  the production build now succeeds cleanly post-split (2,583 modules
  transformed, built in 18.36s) — replacing the single 887kB bundle with
  multiple route-level chunks. Exact per-chunk sizes weren't reported in
  this build's output and are worth a quick look at some point, but the
  code-splitting itself is confirmed working, not just written.
...
- 🟡 **No secret scanning in CI.** Nothing currently prevents an accidental
  commit of a real API key, database URL with embedded credentials, or
  `.env` file contents from landing in git history — the `.gitignore`
  entries prevent it by convention, not by enforcement. A pre-commit or CI
  gate (Gitleaks or TruffleHog, both free and straightforward to add as a
  GitHub Actions job) would catch this before it ever reaches a public
  remote, which matters more here than most projects given this repo is
  intended to be public/open-source.
- 🟡 **No SBOM (Software Bill of Materials) is generated.** For a
  trade-compliance product specifically, supply-chain transparency is a
  reasonable thing for downstream users to expect, not just a nice-to-have.
  CycloneDX has first-class tooling for both ecosystems here
  (`cyclonedx-py` for the Python side, `@cyclonedx/cyclonedx-npm` for the
  frontend) and integrates cleanly as a CI step producing a versioned
...
- 🟡 **No API benchmark report exists.** There's no documented
  requests/second, latency percentile (p50/p95/p99), or memory-under-load
  figures for any endpoint. "Performance" has been evaluated so far only as
  "feels fast enough on a single dev machine" — reasonable during feature
  development, not sufficient as a v1.0.0 claim. A `k6` or `locust` script
  hitting `/classify`, `/duty/calculate`, and `/rulings/search` (the three
  CPU-bound, in-memory-search endpoints) with a realistic concurrency
  profile would turn "should be fast" into an actual measured number worth
  publishing alongside the v1.0.0 release notes.
...
| 17 | No secret scanning in CI | 🟡 | Security |
| 18 | No SBOM generation | 🟡 | Supply chain |
| 19 | No API benchmark report | 🟡 | Performance |
| 20 | on_event/Pydantic Config/datetime.utcnow deprecations | 🟢 | Code quality |
| 21 | No CHANGELOG.md, incomplete release notes as files | 🟢 | Release engineering |
| 22 | Inconsistent API response envelopes | 🟢 | API design |
| 23 | CROSS rulings dataset very small (content, not code) | 🟢 | Content |
...
matching the pattern that's worked well across v0.1–v0.9. Numbering starts
at **v0.9.3**, not v0.9.2: the unreleased dashboard polish pass (dark mode,
toasts, code-splitting, Command Palette, breadcrumbs, error boundary, 404
page, pagination) already occupies the v0.9.2 slot in practice, even though
it hasn't been formally tagged yet.

**v0.9.3 — Security hardening (🔴 items first)**
...
**v0.9.8 — Supply chain & performance validation**
Three concrete, measurable additions before claiming production-readiness:
1. **Secret scanning in CI** (Gitleaks or TruffleHog as a GitHub Actions
   job) — catches accidental credential commits before they reach a public
   remote, which matters more here than most projects given this repo is
   intended to be public.
2. **SBOM generation** (CycloneDX, via `cyclonedx-py` for the backend and
   `@cyclonedx/cyclonedx-npm` for the frontend) — a versioned software
   bill-of-materials artifact per release, reasonable to expect from a
   trade-compliance product specifically.
3. **API benchmark report** — a `k6` or `locust` script exercising
   `/classify`, `/duty/calculate`, and `/rulings/search` under realistic
   concurrency, producing real requests/second, latency percentile
   (p50/p95/p99), and memory-under-load figures to publish alongside the
   v1.0.0 release notes, replacing "should be fast" with an actual number.
...
for the data volume, and cut the first official tagged GitHub Release with
complete notes — including the benchmark figures and SBOM from v0.9.8.
